...
// Notifications contains the notification API
type Notifications struct {
	UserCache *usercache.UserCache
}

// New returns a new Notification instance
func New(userCache *usercache.UserCache) Notifications {
	n := Notifications{userCache}
	return n
}

func (n Notifications) getUserName(id string) (username string) {
	username = n.UserCache.GetUserName(id)
	return
}

func (n Notifications) getUserID(name string) (id string) {
	id = n.UserCache.GetUserID(name)
	return
}

func (n Notifications) link(i string) string {
	return fmt.Sprintf("<@%s|%s>", i, n.getUserName(i))
}

func (n Notifications) item(i queue.Item) string {
	if i.Reason == "" {
		return n.link(i.ID)
	}
...
func (n Notifications) JoinActive(i queue.Item) string {
	return fmt.Sprintf("%s", n.nowHasToken(i))
}

// Leave is a successful leave from the queue
func (n Notifications) Leave(i queue.Item) string {
...
func (n Notifications) LeaveActive(i queue.Item) string {
...
func (n Notifications) LeaveNoEntry(id, reason string) string {
...
func (n Notifications) Done(i queue.Item, q queue.Queue) string {
	a := q.Active()
...
func (n Notifications) DoneNoOthers(i queue.Item) string {
	return fmt.Sprintf("%s\n%s",
		n.finishedWithToken(i), n.upForGrabs())
}

// DoneNotActive tells the user that they must have the token to use done
func (n Notifications) DoneNotActive(user string) string {
...
func (n Notifications) Yield(i queue.Item, q queue.Queue) string {
	a := q.Active()
	return fmt.Sprintf("%s\n%s", n.yielded(i), n.nowHasToken(a))
}

// YieldNoOthers tells the user that they cannot yield if nobody is waiting
func (n Notifications) YieldNoOthers(i queue.Item) string {
...
func (n Notifications) YieldNotActive(i queue.Item) string {
...
func (n Notifications) Barge(i queue.Item, a queue.Item) string {
...
func (n Notifications) Boot(booter string, i queue.Item) string {
...
func (n Notifications) BootNoEntry(id, name, reason string) string {
...
func (n Notifications) OustNoOthers(ouster string, i queue.Item) string {
	return n.ousted(ouster, i)
}

// OustConfirm asks the ouster if they are sure
func (n Notifications) OustConfirm(ouster string, i queue.Item) string {
